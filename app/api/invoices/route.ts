...
export async function POST(request: Request) {
  ...
  if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const { workspaceOwnerId, workspaceUserIds } = await getWorkspaceContext(supabase, user.id);
  ...
  const { count } = await supabase.from("invoices").select("id", { count: "exact", head: true }).in("user_id", workspaceUserIds);
